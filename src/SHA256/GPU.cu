...
__global__ void sha256_cuda_search(int num_chars, char *result, unsigned char *search){
    ...
    int i, divisor = 1;
    int conb = (blockIdx.x * blockDim.x) + threadIdx.x;
...
extern "C" {
    ...
    int find_sha256(int num_chars, unsigned char *search, char *result){
        ...
        int i;
        memset(result, 0xFF, num_chars * sizeof(char));
        char *cuda_result;
        unsigned char *cuda_search;
        int flag = 0;
        checkCudaErrors(cudaMalloc(&cuda_result, sizeof(char) * num_chars));
        checkCudaErrors(cudaMalloc(&cuda_search, sizeof(unsigned char) * SHA256_DIGEST_SIZE_CUDA));
        checkCudaErrors(cudaMemcpy(cuda_result, result, sizeof(char) * num_chars, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(cuda_search, search, sizeof(unsigned char) * SHA256_DIGEST_SIZE_CUDA, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpyToSymbol(finished, &flag, sizeof(unsigned char), 0, cudaMemcpyHostToDevice));
        sha256_cuda_search <<<num_blocks, block_size>>>(num_chars, cuda_result, cuda_search);
        cudaDeviceSynchronize();
        checkCudaErrors(cudaMemcpy(result, cuda_result, sizeof(char) * num_chars, cudaMemcpyDeviceToHost));
        for(i = 0; i < num_chars; i++){
            if(result[i] != 0xFF){
                break;
            }
